...
def complaint():
    if request.method == 'POST':
        ...
        date = datetime.now()
        dic = {
            "name": name, "email": email, "city": city, "ward": ward,
            "mobile": mob, "complain": complain, "status": status,
            "image": img, "date": date
        }
        qq = db.ward
        qw = db.head
        ...
        li = list(qw.find(dw))
        if li:  # Check if the list is not empty
            nn = li[0]["complains"]
            no = nn + 1
            dq = {"complains": no}
...
def update():
    if request.method == 'POST':
        ward = request.form.get('ward')
        city = request.form.get('city')
        dic = request.form.get('dic')  # You should adjust this to get the correct data
        ...
        if li:  # Check if the list is not empty
            ...
            no = nn + 1
            dq = {"complains": no}
            ss.update_one({"complains": nn}, {'$set': dq})
        sa.insert_one(dic)
